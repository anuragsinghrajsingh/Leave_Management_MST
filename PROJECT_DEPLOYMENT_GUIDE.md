...
## 0. Before You Start: Values You Must Collect First

This guide can deploy the project, but it cannot invent company-specific secrets, domain/network details, or server access. Before giving this guide to another person, collect the values below.

If any required value is missing, stop and get it first. Do not guess production secrets.

### 0.1 Required Values Checklist

| Required value | Why it is needed | Who usually provides it | Where it is used |
|---|---|---|---|
| Domain name | Browser users need a public URL | Network/domain team | `DJANGO_ALLOWED_HOSTS`, `DJANGO_CSRF_TRUSTED_ORIGINS`, `PORTAL_BASE_URL`, nginx/SSL |
| SSL/network setup details | HTTPS is required for secure cookies and browser push | Network/security team | nginx config or upstream load balancer/proxy |
| Linux server user | Commands and paths depend on username | Server admin | `/home/<user>/Leave_Management_MST`, systemd `User=` |
| Production project path | Services must point to exact code folder | Server admin/deployer | systemd `WorkingDirectory`, nginx static/media aliases |
| PostgreSQL database name | Django needs DB target | DBA/server admin | `.env` `DB_NAME` |
| PostgreSQL database user | Django needs DB login | DBA/server admin | `.env` `DB_USER` |
| PostgreSQL database password | Django needs DB password | DBA/server admin | `.env` `DB_PASSWORD` |
| PostgreSQL creation access | DB/user must be created before migrate | DBA/server admin | `sudo -u postgres psql` or equivalent |
| `DJANGO_SECRET_KEY` | Django signing/session/security secret | Generated by deployer or provided securely | `.env` `DJANGO_SECRET_KEY` |
| Email SMTP host/port/security | App sends leave/login/report emails | Mail/server admin | `.env` email settings |
| Email mailbox password | SMTP authentication | Mail/server admin | `.env` `EMAIL_HOST_PASSWORD` |
| Leave desk sender email | Official sender address | Company/admin | `.env` `DEFAULT_FROM_EMAIL`, `LEAVE_DESK_FROM_EMAIL` |
| Leave record email(s) | Copy/record recipients for leave emails | Company/admin | `.env` `LEAVE_RECORD_EMAILS` |
| Web Push keys or permission to generate them | Browser notifications need VAPID keys | Existing prod admin or deployer | `.env` `WEB_PUSH_VAPID_*`, `runtime/*.pem` |
| SSL final nginx config ownership | Decide if nginx or network proxy handles certs | Network/server admin | nginx site config |
| GitHub repo access | Needed to fetch/copy production files | GitHub/project owner | `git fetch origin`, `git show origin/main` |

### 0.2 Domain Name

Example current production domain:
...
It is also used in nginx/server/network config:

```nginx
server_name mstleave.mstlabs.in;
```

How to verify DNS from server:

```bash
getent hosts mstleave.mstlabs.in
```

How to verify from browser:

```text
Open https://mstleave.mstlabs.in
```

Do not add random public IPs to `DJANGO_ALLOWED_HOSTS` just because logs show `DisallowedHost`. Direct-IP scans should normally stay blocked.

### 0.3 SSL And Network Setup Details

Production needs HTTPS because:

```text
secure cookies need HTTPS
CSRF trusted origin should match HTTPS domain
browser push notifications require HTTPS outside localhost
users should not send passwords over HTTP
```

There are two common SSL setups.

Option A: nginx owns SSL directly:

```text
Browser -> nginx HTTPS/443 -> gunicorn socket
```

Then nginx needs certificate files, usually from Certbot or company certificate process.

Typical nginx SSL shape:
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/mstleave.mstlabs.in/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/mstleave.mstlabs.in/privkey.pem;
...
Option B: network team/load balancer owns SSL:

```text
Browser -> company SSL/load balancer -> nginx/gunicorn
```

Then ask network team:

```text
Is SSL terminated before nginx?
What headers are forwarded?
Should nginx listen only HTTP internally?
What domain will users use?
Is HTTP redirected to HTTPS at network layer?
```

Django `.env` should still use the HTTPS public domain:
...
### 0.4 Server Username And Production Path

Current production user/path:

```text
Linux user: mstleave
Project path: /home/mstleave/Leave_Management_MST
```

If another server uses a different user, replace every path in systemd and nginx.

Example if user is `leaveapp`:

```text
/home/leaveapp/Leave_Management_MST
```

Then systemd must also change:

```ini
User=leaveapp
WorkingDirectory=/home/leaveapp/Leave_Management_MST
ExecStart=/home/leaveapp/Leave_Management_MST/venv/bin/python /home/leaveapp/Leave_Management_MST/manage.py qcluster
```

And nginx aliases must change:

```nginx
alias /home/leaveapp/Leave_Management_MST/staticfiles/;
alias /home/leaveapp/Leave_Management_MST/media/;
```
...
### 0.5 PostgreSQL Database Name/User/Password

Django needs a PostgreSQL database and user before migrations run.
...
Example creation flow if you have PostgreSQL admin access:
...
CREATE USER leave_user WITH PASSWORD 'replace-this-db-password';
ALTER ROLE leave_user SET client_encoding TO 'utf8';
ALTER ROLE leave_user SET default_transaction_isolation TO 'read committed';
ALTER ROLE leave_user SET timezone TO 'Asia/Kolkata';
GRANT ALL PRIVILEGES ON DATABASE leave_management TO leave_user;
\q
```

For PostgreSQL 15/16, you may also need schema privileges:

```bash
sudo -u postgres psql -d leave_management
```
...
ALTER SCHEMA public OWNER TO leave_user;
\q
```

Then test Django DB connection:
...
If database password changes later:

```text
1. Change password in PostgreSQL.
2. Update .env DB_PASSWORD.
3. Restart gunicorn, qcluster, lms_scheduler.
4. Run python manage.py check.
```

### 0.6 `DJANGO_SECRET_KEY`

This is a private Django signing secret. It must be long, random, and kept secret.

Generate using project helper:
...
```env
DJANGO_SECRET_KEY=generated-long-secret-value
```

Do not commit it to git.

If changed on an existing production system:

```text
existing sessions may become invalid
some signed tokens may become invalid
restart gunicorn/qcluster/lms_scheduler after update
```

### 0.7 Email SMTP Details And Mailbox Password

The app sends leave emails, login/security emails, admin bulk emails, and weekly reports.
...
EMAIL_HOST_PASSWORD=real-mailbox-password
...
EMAIL_PORT: SMTP port, often 465 for SSL or 587 for TLS
EMAIL_USE_SSL: True for port 465
EMAIL_USE_TLS: True for port 587
EMAIL_HOST_USER: login username/mailbox
EMAIL_HOST_PASSWORD: mailbox/app password
DEFAULT_FROM_EMAIL: default sender
LEAVE_DESK_FROM_EMAIL: leave-desk sender address
LEAVE_RECORD_EMAILS: record/copy recipients, comma-separated
```
...
Do not set both SSL and TLS true together.
...
Test through the app/admin/service console, then check email logs.

### 0.8 Web Push Keys Or Permission To Generate Them

Web Push browser notifications use VAPID keys.
...
Secret key used by server to send push notifications.
Keep it private.

WEB_PUSH_VAPID_SUBJECT
Contact identity for push sender, usually mailto:admin@example.com.
```

Generate on fresh production:
...
Important for existing production:

```text
Do not regenerate Web Push keys unless you accept that existing browser subscriptions may stop working.
Users may need to enable/subscribe to notifications again.
```

So for existing production, get the current keys from the current server/admin.

For fresh production, generate new keys.

### 0.9 GitHub Repository Access

A deployer needs access to fetch the repository:

```bash
git fetch origin
```

The current production layout is flattened, so production update commands use:

```bash
git show origin/main:"Leave Management/path/in/repo" > path/in/production
```

If a new deployer clones fresh and chooses to keep the repository layout exactly, they must adjust paths, systemd `WorkingDirectory`, nginx aliases, and commands accordingly.

Current guide assumes:

```text
Production contains the contents of Leave Management/ directly inside /home/mstleave/Leave_Management_MST.
```

### 0.10 Final Pre-Deployment Stop Check

Before running deployment commands, confirm these are known:

```text
[ ] Domain name
[ ] SSL ownership: nginx or network team/load balancer
[ ] Linux username
[ ] Production project path
[ ] PostgreSQL DB name
[ ] PostgreSQL DB user
[ ] PostgreSQL DB password
[ ] PostgreSQL admin access or DBA support
[ ] DJANGO_SECRET_KEY
[ ] Email SMTP host/port/security mode
[ ] Email mailbox username/password
[ ] Leave desk sender email
[ ] Leave record recipient email(s)
[ ] Web Push keys or permission to generate new ones
[ ] GitHub repository access
[ ] Decision about production layout: flattened or same as repo
```

If any required item is unknown, stop and collect it first.
...
### 35.6A Holiday Import And Sync Commands

Use management commands for holiday work:

```bash
python manage.py import_holidays path/to/holidays.csv
python manage.py sync_public_holidays
```

Do not use old direct helper scripts blindly on production. In particular, direct scripts under `App/scripts/` may be development helpers and may not have the correct production settings module or safety prompts.
...
## 36. Documentation Update System For Future Features

This section explains exactly how to update documentation when a new feature, service, environment variable, or production behavior is added.

### 36.1 Documentation File Roles

```text
README.md
Public/current project overview. Keep it readable for someone seeing the project first.

PROJECT_START_HERE.md
Fast daily memory. Commands, where to look, what service owns what.

PROJECT_GUIDE.md
Practical feature guide. Explain how features work from Admin/HR/Employee point of view.

PROJECT_DEPLOYMENT_GUIDE.md
Production deployment guide. Linux services, nginx, workers, .env, migrations, collectstatic, backup, logs.

PROJECT_DEEP_DIVE_BOOK.md
Large technical/debug book. Add current-state addendums or regenerate when doing full documentation rebuild.

Leave Management/App/management/production_setup/README.md
Production secret/key generation guide.
```

### 36.2 When A New Feature Is Added

Update docs based on what changed:

```text
New user-facing feature
-> README.md if important enough
-> PROJECT_GUIDE.md for workflow
-> PROJECT_START_HERE.md if users need a quick command/path

New admin feature
-> PROJECT_GUIDE.md admin section
-> PROJECT_DEEP_DIVE_BOOK.md current addendum if debugging is important

New production service or Linux tuning
-> PROJECT_DEPLOYMENT_GUIDE.md
-> PROJECT_START_HERE.md service map

New env variable
-> .env.example
-> README.md if common setup value
-> PROJECT_DEPLOYMENT_GUIDE.md env section
-> production_setup/README.md if generated/secret-related

New migration/model/table
-> PROJECT_GUIDE.md model/feature section
-> PROJECT_DEPLOYMENT_GUIDE.md deployment decision table if deploy behavior changes

New static JS/CSS behavior
-> PROJECT_GUIDE.md feature section
-> PROJECT_DEPLOYMENT_GUIDE.md if collectstatic/deploy behavior changes

New background job or scheduler job
-> PROJECT_GUIDE.md background/scheduler section
-> PROJECT_DEPLOYMENT_GUIDE.md service/restart/scheduler sections
-> PROJECT_START_HERE.md service ownership map
```

### 36.3 Required Feature Documentation Template

Use this block for every meaningful feature/change:
...
### 36.4 Required Production Incident Template

Use this block when documenting a production problem we faced:
...
### 36.5 Required Deployment Entry Template

Use this block after every production deploy:
...
collectstatic run: yes/no
Services restarted:
Health checks:
Browser/admin test performed:
Logs checked:
Result:
Follow-up needed:
```

### 36.6 Do Not Let Docs Drift

Documentation drift happens when the code changes but the guide still says the old behavior.

Before saying a feature is done, check:

```text
1. Is README still true?
2. Is START_HERE still useful?
3. Does PROJECT_GUIDE explain the new behavior?
4. Does PROJECT_DEPLOYMENT_GUIDE explain deployment/restart/env/static impact?
5. Does DEEP_DIVE need a current addendum?
6. Does production_setup README need secret/env changes?
```

### 36.7 Current Feature Areas That Must Always Stay Documented

```text
Leave apply/edit/delete rules
HR approve/reject workflow
Admin leave workflow
Leave balance and WFH bridge behavior
Notification read/seen behavior
Communication center behavior
Email recipients and record email behavior
Admin bulk email jobs and recovery
Login lock counters
Password max length and password reset behavior
Backup/restore behavior
Scheduler jobs
qcluster behavior
Static/media production behavior
Linux service setup
Dev vs production differences
Security checks
```

### 36.8 Quick Doc Verification Commands
...
Find stale old env names:

```bash
rg -n "SECRET_KEY=|DEBUG=True|ALLOWED_HOSTS=|DATABASE_URL|EMAIL_USE_TLS=True" *.md "Leave Management/App/management/production_setup/README.md"
```

Find current env names:

```bash
rg -n "DJANGO_SECRET_KEY|DJANGO_DEBUG|DJANGO_ALLOWED_HOSTS|LEAVE_RECORD_EMAILS|PASSWORD_INPUT_MAX_LENGTH|SHORT_HALF_LEAVE|SICK_LEAVE" *.md "Leave Management/App/management/production_setup/README.md"
```

Find service references:

```bash
rg -n "gunicorn|qcluster|lms_scheduler|nginx|postgresql|collectstatic|migrate" *.md
```

## 37. Production Verification Workbook

Use this workbook after any deployment that touches production behavior.

### 37.1 Basic Verification
...
CSRF failure
DisallowedHost from real domain
```
...
```text
Open portal.
Login with relevant role.
Test exact feature changed.
Hard refresh if static changed.
```

### 37.3 Background Verification
...
```text
qcluster active
around 6 qcluster processes
no repeated crash/reincarnation loop
```

### 37.4 Scheduler Verification
...
SCHEDULER | STARTED
JOB_ADDED lines
Keep-alive lines over time
```

### 37.5 Backup Verification
...
## 38. Rollback Playbooks

Rollback means returning production to the previous working version. Use carefully.

### 38.1 Roll Back One Copied File

If only one file was copied from GitHub and it broke production:
...
Find previous commit hash from GitHub history, then copy previous version:

```bash
git show PREVIOUS_COMMIT:"Leave Management/App/views.py" > App/views.py
python manage.py check
sudo systemctl restart gunicorn
```

### 38.2 Roll Back Static File

```bash
git show PREVIOUS_COMMIT:"Leave Management/static/js/my_leave.js" > static/js/my_leave.js
python manage.py collectstatic
sudo systemctl restart gunicorn
```
...
Also restart qcluster/scheduler if the env value affects them:
...
### 38.4 Roll Back Migration Change

Database rollback is risky.

Do not reverse migrations casually on production. First decide:

```text
Was data written into the new table/field?
Can old code run with the new schema?
Is a backup needed before rollback?
Should maintenance mode be enabled?
```

Safer pattern:

```text
If possible, deploy code fix that works with current schema instead of reversing DB.
```

### 38.5 Emergency Service Restart

If a service is unhealthy but code is correct:
...
## 39. Deployment Examples For Common Changes

### 39.1 Login Page Template Change
...
### 39.2 Password Security Code Change
...
### 39.3 Leave Apply Frontend Change
...
### 39.4 Scheduler Job Change

```bash
git fetch origin
git show origin/main:"Leave Management/App/services/scheduler.py" > App/services/scheduler.py
git show origin/main:"Leave Management/App/management/commands/run_lms_scheduler.py" > App/management/commands/run_lms_scheduler.py
python manage.py check
sudo systemctl restart lms_scheduler
sudo journalctl -u lms_scheduler -n 100 --no-pager
```

### 39.5 Admin Email Job Change
...
If static admin status JS changed:

```bash
git show origin/main:"Leave Management/static/admin/js/admin_email_job_status.js" > static/admin/js/admin_email_job_status.js
python manage.py collectstatic
sudo systemctl restart gunicorn
```
